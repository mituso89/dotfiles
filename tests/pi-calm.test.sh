...
# Home Manager modules: the shared one plus both platform entry points.
HOME_MODULES="$ROOT/home-common.nix $ROOT/home.nix $ROOT/home-linux.nix"
...
test_zero_coupling_and_state_file() {
  ...
  for file in $source_files "$ROOT/tests/pi-calm.test.sh" "$ROOT/tests/lib.sh" "$ROOT/README.md" $HOME_MODULES; do
  ...
  license_hits=$(grep -rni "$attribution_name" "$CALM_DIR" "$ROOT/README.md" $HOME_MODULES 2>/dev/null | grep -v "Adapted from" || true)
  ...
  assert_not_contains "$(cat $HOME_MODULES)" '.pi/agent/calm' "a Home Manager module manages the Calm state file"
...
test_static_typescript_and_repo_wiring() {
  ...
  grep -q 'home.file.".pi/agent/extensions".source =' "$ROOT/home-common.nix" \
    || fail "home-common.nix no longer links ~/.pi/agent/extensions as a directory"
  grep -q "mkOutOfStoreSymlink \"\${dotfiles}/home/.pi/agent/extensions\"" "$ROOT/home-common.nix" \
    || fail "home-common.nix changed the Pi extensions link target"
